...
# Instagram requests need spoofing
insta = req.Session()
...
def get_code_from_mail(token, subject_keyword="Instagram"):
    ...
    for attempt in range(30):
        try:
            ...
            print("⚠️ Error while polling mail.tm:", e)
            time.sleep(2)
    return None
...
def create_account(headers, email, signup_code):
    ...
    if '"account_created":true' in r.text:
        ...
        print(f"🍪 Session ID: {r.cookies.get('sessionid')}")
        print(f"📛 CSRF Token: {r.cookies.get('csrftoken')}")
        print(f"🧠 DS User ID: {r.cookies.get('ds_user_id')}")
    else:
...
if __name__ == "__main__":
    ...
    if '"email_sent":true' in response:
        ...
        if code:
            ...
            print("❌ No code received in inbox.")
    else:
